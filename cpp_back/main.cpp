...
void draw_pango_text(PixelBuffer &buffer, const std::string &text, int width, int height, int x, int y, int font_size = 64)
{
    ...
    cairo_set_source_rgba(cr, 0, 0, 0, 1); // black background
    cairo_paint(cr);
    ...
    std::string font_desc_str = "DejaVu Sans Bold " + std::to_string(font_size);
        ...
        threads.emplace_back([&]()
            ...
            while (true) {
                ...
                static std::mutex ft_mutex;
                {
                    ...
                    draw_pango_text(buffer, display_text, width, height, x, y, text_height); //pangocairo text renderin
                    //draw_text(face, buffer, display_text, x, y, 255, 255, 255, width, height); //freetype text rendering
                }
