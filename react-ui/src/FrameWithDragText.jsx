...
const FrameWithDragText = ({ totalFrames = 300, fps = 30, sendRenderRequest }) => {
    const getFrameUrl = (index) => {
        ...
        return `http://localhost:8000/frames/frame_${padded}.png?t=${Date.now()}`;
    };
    ...
    const [currentSrc, setCurrentSrc] = useState(getFrameUrl(0)); // source shown to the user
    const [nextSrc, setNextSrc] = useState(null); // preload src

    // This ensures the image reloads when frameIndex changes
    useEffect(() => {
        setIsImageLoaded(false);
        setImageError(false);
        setImageKey(prev => prev + 1); // Force remount of image component
    }, [frameIndex]);

    // Add this to preload adjacent frames
    useEffect(() => {
        const newSrc = getFrameUrl(frameIndex);
        const preloadImage = new Image();
        //setNextSrc(newSrc);

        preloadImage.src = newSrc;
        preloadImage.onload = () => {
            console.log(`Preloaded frame ${frameIndex}`);
            setCurrentSrc(newSrc); // swap only after it's fully loaded
            setIsImageLoaded(true);
        };
        preloadImage.onerror = (err) => {
            console.error('Image load failed for frame:', frameIndex, err);
        };
    }, [frameIndex]);
    /*
    useEffect(() => {
        const preloadNext = new Image();
        preloadNext.src = getFrameUrl((frameIndex + 1) % totalFrames);
        
        const preloadPrev = new Image();
        preloadPrev.src = getFrameUrl((frameIndex - 1 + totalFrames) % totalFrames);
    }, [frameIndex]);
    */
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        preload.src = getFrameUrl(nextIndex);
    }, [frameIndex]);
                    ...
                    >
                        <input
                            ...
                            style={{
                                ...
                                fontFamily: fontUrl ? fontName : 'arial',
                                border: 'none',
                                outline: 'none',
